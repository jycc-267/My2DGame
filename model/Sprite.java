...
// classes for raster implementation of draw(). See ShieldMushroom, Bullet, or Asteroid for vector implementations of
// draw().
@Data
public abstract class Sprite implements Movable {
    ...
    //what direction a sprite is moving and how fast it is moving
    private double deltaX, deltaY;

    //every sprite has a team: friend, foe, floater, or debris.
    private Team team;
    //every sprite is an object circumscribed by a circle
    //the radius of circumscribing/inscribing circle
    private int radius;

    //orientation from 0-359
    private int orientation;
    //natural mortality (short-lived sprites only, for example a shooting bullet doesn't last forever and should be removed from the game panel)
    //it's also a number of frame that a sprite can live
    private int expiry;
    ...
    //Once set, their values do not change.
    //It's the job of the renderVector() method to adjust for orientation and location.
    ...
    public void move() {
        ...
        //To ensure this behavior among all sprites in your game, make sure to call super.move() in extending classes where you need to override the move() method.
        //A sprite should know the bound of the game frame so that it may be able to roll over to another side when reaching the bound
    ...
    protected void expire() {
        ...
        //if a short-lived sprite has an expiry of one
        //it commits suicide by enqueuing itself (this) onto the opsList with an operation of REMOVE
        if (expiry == 1) {
